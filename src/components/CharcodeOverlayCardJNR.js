...
const CharcodeOverlayCardJNR = ({ parsed, onClose }) => {
  ...
  const [ebcHistory, setEbcHistory] = useState([]);
   ...
   useEffect(() => {
    ...
    fetchJnrOverlayData(user, bagDate, charcodeId)
      ...
      .catch((err) => {
        console.error('CharcodeOverlayCard (JNR) fetch error:', err);
      });
  }, [bagDate, charcodeId, user]);

  function handleDeleted(deletedDate, deletedTime) {
  setEbcEntries(entries =>
    entries.filter(
      e => !(e.date === deletedDate && e.time === deletedTime)
    )
  );
}
