...
const LoginScreen = ({ onLogin }) => {
  ...
  const [highlighedtInput, setHighlightedInput] = useState(null);

  const login = () => {
    // ⚠️ Replace with real validation logic later
    if (email && password) {
      onLogin(); // Tell App.js we're authenticated
    } else {
      setHighlightedInput("email"); // Or "password"
    }
  };
