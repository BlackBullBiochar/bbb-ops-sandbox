...
router.get('/', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Failed to fetch charcodes' });
  }
});
...
router.get('/data/by-file/:filename', async (req, res) => {
  try {
    ...
    const doc = await Charcodes.findOne({ filename: fn }).lean();
...
router.delete('/data/by-file/:filename', async (req, res) => {
  try {
    const fn = decodeURIComponent(req.params.filename);
    const result = await Charcodes.deleteOne({ filename: fn });
...
// PATCH /api/charcodes/update-status
  ...
  try {
    ...
    for (const doc of docs) {
      const index = doc.data.findIndex(row =>
        row['ID'] === charcodeId || row['Charcode ID'] === charcodeId
      );
      ...
      if (index !== -1) {
        ...
        return res.json({
          message: 'EBC status updated',
          updatedRow: doc.data[index]
        });
      }
    }
